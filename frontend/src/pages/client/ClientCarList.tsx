...
const ClientCarList: React.FC = () => {
  ...
  const { data: carsResponse, isLoading, error } = useQuery({
    queryKey: ['cars', page, searchTerm, manufacturer, model, year, color, priceRange],
    queryFn: () => carService.getCars(page, 12, {
  ...
  const { data: modelsByManufacturerResponse } = useQuery({
    queryKey: ['modelsByManufacturer', manufacturer],
    queryFn: () => carService.getModelsByManufacturer(manufacturer),
